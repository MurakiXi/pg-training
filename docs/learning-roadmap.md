...
### Week7 Day6-5：問題追加上限のヘッダー連動

- 問題追加上限 `questionLimit` を `QuestionForm` 内の固定値から `App` 側へ移し、アプリ全体のルールとして一元管理した。
- `QuestionForm` に `questionLimit: number` Propsを追加し、上限判定に親から渡された値を使用するよう変更した。
- `App` で `remainingQuestionCount = questionLimit - questions.length` を導出し、Stateを追加せず現在の問題数から残り追加可能数を算出した。
- `QuizHeader` に `questionsLength` と `remainingQuestionCount` を渡し、「現在○問・あと○問追加できます」という情報を表示した。
- 残り追加可能数が0になった場合は、「これ以上は問題を追加できません」という表示へ切り替えた。
- 問題数が上限に達し、かつ追加フォームが閉じている場合のみ、フォーム表示ボタンを `disabled` にした。
- 9問の状態で追加フォームを開き、そのまま10問目を追加した場合でも、フォームを閉じる操作は可能なままになることを確認した。
- 10問到達後にフォームを閉じると、再度フォームを開けなくなることを確認した。
- 上限値の二重管理を避け、ルールの出所を `App` に集約する設計を確認した。
- Stateにすべき値と既存値から計算できる導出値の違い、親子コンポーネント間での責務分担を復習した。
